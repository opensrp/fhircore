...
A PlanDefinition is a pre-defined set of actions, often conditional and based on certain decision points (triggers and conditions). Each action defines an activity to be performed often in terms of an [ActivityDefinition](http://hl7.org/fhir/activitydefinition.html) (via definition and/or transform).
...
- `cod`e : CodeableConcept for standard code for this activity. 
...
- `timing`: If dosage is define this can not be use. This is an alternate way of defining timing for Task. The timing can define a complete schedule
  ...
  example: 
    - ANC PlanDefinition generating all Tasks at once
      ```json
        {
          "resourceType": "PlanDefinition",
          "id": "132157",
      ...
      ```
    - ANC PlanDefinition generating  Tasks at demand 
      ```json
        {
          "resourceType": "PlanDefinition",
          "id": "132157",
