...
Since CQL itself is a user-friendly script rather than a parseable configuration or program, it needs a `JSON` or `XML` form that can be lexically parsed and understood by the parser. Thus, it is transformed into a JSON form called Expression Logical Model(ELM) aka `json+elm`. An XML form `xml+elm` is also used. In most cases, we shall be using `json+elm` for parsing FHIR Core libraries. 

The translated `elm` and `cql` is then added to FHIR [Library](http://hl7.org/fhir/R4/library.html) resource as Base64 encoded data to allow the code to get the calculation for each variable and be able to be used by FHIR compatible software.
...
FHIR Core tooling library can also be used to get a Library by running commands
```
fct convert -t cql -i /some/path/Patient-1.0.0.cql
```

Details on the tool can be found [here](https://github.com/opensrp/fhircore-tooling#converting-library-cql-to-json)
...
**Note** : The above approaches output a `json-elm` which then needs to be Base64 decoded and copied to the [Library](http://hl7.org/fhir/R4/library.html) content as Attachment.