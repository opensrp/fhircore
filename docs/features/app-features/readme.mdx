...
Patients can be searched by NAME or ID. This is a manual search where the patient list is updated after two characters are entered (the results update without pressing "enter"). The search will display patients only assigned to the health worker. If you have enabled the mapping extension and enter a search in the patient list view, then switch to the map view, the search filter will continue to apply to the map view.
