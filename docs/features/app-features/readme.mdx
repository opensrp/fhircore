...
Through sync insights OpenSRP makes it possible to understand which data has been synced back to the remote server and what data remains unsynced on your device.  These insights help provide assurance data is synced securely when it's should. It also helps the health worker navigate / troubleshoot data connectivity issues and is invaluable to teams when providing remote support.
