...
1. Open and merge a PR to update the project version to the current release version
    * e.g. change the `versionName` in the Quest [build.gradle](https://github.com/opensrp/fhircore/blob/main/android/opensrp/build.gradle#L28) file to `v0.2.0`
...
App name|`android/opensrp/src/new-flavor/res/values/strings.xml`
...
The following are all valid tags that will trigger the generation of the Quest release APK
> - opensrp-1.2.0
> - opensrp-1.2.0-alpha
> - opensrp-1.2.0-beta
> - opensrp-1.2.0-rc1

**Note:** When creating a tag for the _Quest OpenSRP Version 1.2.0 RC 5 release variant_, use the command:

```
git tag -a opensrp-1.2.0-rc5 -s && git push origin opensrp-1.2.0-rc5
```
...
For example, to delete the opensrp-1.2.0-rc1 tag run the command:

```
git push --delete origin opensrp-1.2.0-rc1 && git tag --delete opensrp-1.2.0-rc1
```