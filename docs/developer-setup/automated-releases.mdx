...
* The APK flavor as set up in the gradle build configuration : _quest_
...
**Note:** The '-quest' section above specifies the flavor to build. When creating a tag for the _Quest OpenSRP Version 1.2.0 RC 5 release variant_, use the command:

```
git tag -a v1.2.0-quest-rc5 -s && git push origin v0.2.1-quest-rc5
```
...
For example, to delete the v1.2.0-quest-rc1 tag run the command:

```
git push --delete origin v1.2.0-quest-rc1 && git tag --delete v1.2.0-quest-rc1
```
