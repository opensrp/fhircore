...
Currently the codebase supports building a release that uses the [Proxy/Gateway](https://github.com/opensrp/fhir-gateway) and one that doesn't. Proxy is default but if you backend server is not using the [FHIR Gateway](https://github.com/opensrp/fhir-gateway/releases) you need to specify this in the build configuration. For now, inorder to switch between the two one needs to update [this file here](https://github.com/opensrp/fhircore/blob/main/android/engine/build.gradle.kts#L30) and set the value to `true` 
