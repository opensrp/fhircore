...
Using $lite to generate such tasks by linking the correct StructureMap in the action to execute task generation dynamically based on number children born. A sample plan definition activity definition action is shown below;
