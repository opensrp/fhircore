...
The data migration configuration outlines a set of rules and operations for migrating FHIR resources eg `Task` resource migration. With this configuration one is able to filter the target resource eg `Encounter`, `Task`` e.t.c`and  read its referenced `PlanDefinition` and its careplans then filter for the task or resource being migrated that lies within the careplan period and read the resulting careplan id . it cleans out the malformed resource attributes that a wrong `StructureMap` may have generated.
