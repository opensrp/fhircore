...
The below image shows how different components are connected to work together with a list widget and how it looks like.

![list](/img/config_types/list_ui.png)
...
### Goals of List Widget
   - Displays of listed data
   - Supports pagination of data
   - Gives ability to scroll multiple data.
...
The below json code defines the configuration properties for a Compound Text view, which is a view that displays two text strings with an optional separator between them. This view can also have actions associated with each text string.
...
### Goals of Compound Text Widget
 - Display text of different properties


## How to use BUTTON

![button](/img/config_types/button_ui.png)

Buttons are often used to perform actions when clicked, in fhircore the same use case applies most actions are performed on button click.

The json below defines the configuration properties of a Button view, which is a view that can do some actions when clicked. Additionally, the button can be styled using available properties such as `status`, `contentColor`, and `buttonType`
 ``` json
 [
   {
     "viewType": "BUTTON",
...
### Goals of a Button
  ...
  - Buttons provides the capability to launch a new action like logging in into the app.
  - Buttons are used to complete an ongoing action like saving input user credentials to the database

## SERVICE_CARD widgets

![serviceCard](/img/config_types/service_card_ui.png)

Service Card displays Patient's details ,other related services and tasks. The details field within the SERVICE_CARD display patient's name and ID.
...
### Goals of COLUMN
 -  To arrange data or even views in a vertical manner.
...
![divider](/img/config_types/divider.png)
...
### Goals of a card widget
    -   Card widgets are used to group similar data together under a single view. Example when you want user contact details to be under a centralised place you can group those information like username, age, phone number, and country of residence under a single card.