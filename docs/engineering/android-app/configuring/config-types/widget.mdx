...
##How to use ACTIONABLE BUTTON
Defines the configuration properties for a Actionable Button view, which is a view that displays button with text strings, it's status ,visibility mode, and if it is enabled. This view can also have actions associated with it.
...
### Config properties of ACTIONABLE BUTTON
|Property | Description | Required | Default |
|--|--|:--:|:--:|
...
|status| status is the current mode of the button it can be a pure text like `Started` or even the color code of different modes|Yes| |
|visible| This is the state of the button that is if it should be shown or not|No| |
|enabled| enabled is used to set if the button is active |Yes| "true"|
