...
status | status indicate the service Status of the action that the button is supposed to perform this service status can be `DUE`, `OVERDUE`, `UPCOMING`,`COMPLETED`, `IN_PROGRESS`, `EXPIRED` | Yes | |
visible | The state whether a button should be shown or hidden | No | |
enabled | enabled is used to set if the button is active that is if a button is set to perform the required action by the user | Yes | "true" |
clickable | clickable is used to set if the button is allowed to perform its action on click or not | No | |
smallSized | smallSized determines if the button needs to be shown small size  | No | |
...
startIcon | startIcon is used to specify an icon within the button if it is needed by default it is always null |  | null |
letterSpacing | letterSpacing is used to specify the amount of spacing that should be between the button text |  | 0 |
buttonType | the buttonType specifies the size of the button it can me `MEDIUM`,`TINY`,`BIG` |  | ButtonType.MEDIUM |
