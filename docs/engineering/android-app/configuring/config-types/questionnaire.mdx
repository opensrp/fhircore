...
type | Creates various item types e.g. date, choice or a group. A group type allows you to create a section within a questionnaire that controls and treat the items within the group as a single item. This is suitable for block sections that can be toggled based on a condition or answer expression logic | no | null
repeats | controls an item to either as a repeated item on not, this can be used to control a single item or group item into a repeated group | no | false
