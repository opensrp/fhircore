...
There are many ways to validate questionnaire components and widgets e.g. `date-picker`, `numeric field` among others, one of the ways to achieve thise is through the use
of `StructureDefinition` extensions to set value constraints.

## Validation on questionnaire's Date picker

Date picker is a widget that support date selection, you can specify the  minimum and maximum dates to act as a range of dates to pick from.

Both Maximum and Minimum date can be any date either in the future or past e.g. three month to come, three months past or even today. This set the date upper limit to the specified value

It is required that the `minDate` is set as an earlier/lower date to `MaxDate`
