...
- Retrieving of th latest migration version from shared preferences
- Performing the migration only if the new version is greater than or equal to the latest migration version

The data migration configuration outlines a set of rules and operations for migrating FHIR resources , specifically `Tasks`migration. With this configuration one is able to filter the target task, read its referenced `plandef` and `rev_including` its careplans then filter for the task that lies within the careplan period and read the resulting careplan id then use it to update the `task.basedOn`.It ensures that all the resources that are being generated through structuremaps are not malformed.
