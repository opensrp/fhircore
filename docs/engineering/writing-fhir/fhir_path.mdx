...
- It allows the retrieval of FHIR resources from the database when the questionnaire is opened using X-FHIR-Query. The retrieved information can then be utilized within the questionnaire through other variables or extensions that utilize expressions
