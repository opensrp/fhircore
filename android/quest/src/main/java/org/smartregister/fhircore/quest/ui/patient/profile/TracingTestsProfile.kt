        ...
        Scaffold(modifier = Modifier.fillMaxWidth(), topBar = {
            ...
            Column(Modifier.fillMaxWidth()) {
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Button(onClick = { viewModel.updateUserWithTracing(isHomeTracing = true) }, enabled = !isOnTracing) {
                        Text(text = "Add Home Tracing")
                    }
                    Button(onClick = { viewModel.updateUserWithTracing(isHomeTracing = false) }, enabled = !isOnTracing) {
                ...
                items(TracingTestsViewModel.testItems) { item ->
                    when (item) {
                        ...
                        is TestItem.QuestItem -> {
                            Card(onClick = { viewModel.open(context, item) }, border = BorderStroke(1.dp, MaterialTheme.colors.secondary), elevation = 0.dp, contentColor = MaterialTheme.colors.onSurface, modifier = Modifier.fillMaxWidth()) {
...
fun QuestContainer(title: String, list: List<String>) {
    val annotatedString = buildAnnotatedString {
        ...
        list.forEach { item ->
            withStyle(style = if (!item.contains("-")) SpanStyle(background = MaterialTheme.colors.primary, color = MaterialTheme.colors.onPrimary) else SpanStyle(background = MaterialTheme.colors.error, color = MaterialTheme.colors.onError)) {
                append(item.replace("-", ""))
            }
            append(" ")
        }
