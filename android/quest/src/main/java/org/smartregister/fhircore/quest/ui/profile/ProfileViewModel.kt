    ...
    when (event) {
      is ProfileEvent.OverflowMenuClick -> {
        ...
        viewModelScope.launch {
          actions?.run {
            find { actionConfig -> actionConfig.interpolate(event.resourceData?.computedValuesMap ?: emptyMap()).workflow?.let { workflow -> ApplicationWorkflow.valueOf(workflow) } == ApplicationWorkflow.CHANGE_MANAGING_ENTITY }
              ?.let {
