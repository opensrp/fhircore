...
class RecordVaccineActivity : QuestionnaireActivity() {
    ...
    override fun handleQuestionnaireResponse(questionnaireResponse: QuestionnaireResponse) {
        lifecycleScope.launch {
            clientIdentifier?.let { identifier: String ->
                (questionnaireViewModel as RecordVaccineViewModel).getVaccineSummary(identifier)
                    .observe(this@RecordVaccineActivity) { vaccineSummary: PatientVaccineSummary? ->
                            ...
                            lifecycleScope.launch {
                                questionnaire?.let { questionnaire ->
                                    questionnaireViewModel.performExtraction(
                                        ...
                                        val immunizationEntry =
                                            entry.firstOrNull { it.resource is Immunization }
    ...
    private fun showVaccineRecordDialog(
        ...
        val nextVaccineDate = DateUtils.addDays(vaccineDate, 28, dateTimeFormat = "MMM d, yyyy h:mm:ss a")
