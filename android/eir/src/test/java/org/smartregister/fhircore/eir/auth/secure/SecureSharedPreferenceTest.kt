...
package org.smartregister.fhircore.eir.auth.secure
...
class SecureSharedPreferenceTest : RobolectricTest() {
  ...
  private val secureConfig = SecureSharedPreference(context)

  private val testSharedPreferences =
    EncryptedSharedPreferences.create(
      context,
      SecureSharedPreference.SECURE_STORAGE_FILE_NAME,
      testMasterKey,
  ...
  fun testSaveCredentials() {
    val credentials = AuthCredentials("testuser", "testpw".toCharArray(), "my-token")
    val credentialsExpectedStr = Gson().toJson(credentials)

    secureConfig.saveCredentials(credentials)

    val credentialsSavedStr =
      testSharedPreferences.getString(
        SecureSharedPreference.KEY_LATEST_CREDENTIALS_PREFERENCE,
        null
      )
  ...
  fun testRetrieveCredentialsTokenShouldReturnCorrectCredentials() {
    ...
    testSharedPreferences.edit {
      putString(SecureSharedPreference.KEY_LATEST_CREDENTIALS_PREFERENCE, credentialsExpectedStr)
    }
  ...
  fun testDeleteCredentials() {
    ...
    testSharedPreferences.edit {
      putString(SecureSharedPreference.KEY_LATEST_CREDENTIALS_PREFERENCE, credentialsStr)
    }
