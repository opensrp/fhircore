    ...
    companion object {
        fun get(
            ...
            return ViewModelProvider(
                owner,
                BmiComputeViewModel(application, repository).createFactory()
...
//    }
    ...
    fun getBMIResult(computedBMI: Double, activityContext: Context): SpannableString {
        ...
        val mGreenSpannedText =
            ForegroundColorSpan(getBmiResultHighlightColor(matchedCategoryIndex))
    ...
    private fun getStartingIndexInCategories(index: Int, activityContext: Context): Int {
        return when (index) {
            1 -> getBMICategories(activityContext).indexOf(activityContext.getString(R.string.bmi_category_1))
            2 -> getBMICategories(activityContext).indexOf(activityContext.getString(R.string.bmi_category_2))
            3 -> getBMICategories(activityContext).indexOf(activityContext.getString(R.string.bmi_category_3))
            else -> getBMICategories(activityContext).indexOf(activityContext.getString(R.string.bmi_category_4))
        }
    }

    private fun getEndingIndexInCategories(index: Int, activityContext: Context): Int {
        return when (index) {
            1 -> getStartingIndexInCategories(
                1,
                activityContext
            ) + activityContext.getString(R.string.bmi_category_1).length
            2 -> getStartingIndexInCategories(
                2,
                activityContext
            ) + activityContext.getString(R.string.bmi_category_2).length
            3 -> getStartingIndexInCategories(
                3,
                activityContext
            ) + activityContext.getString(R.string.bmi_category_3).length
            else -> getStartingIndexInCategories(
                4,
                activityContext
