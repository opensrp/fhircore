...
fun EncounterItem(
  ...
  Column {
    Row(
      ...
      // icon
      Icon(Icons.Filled.TaskAlt, null)
...
fun EncounterListScreenPreview() {
  AppTheme {
    EncounterListScreen(dummyData())
  }
}

fun dummyData() = object : EncounterDataProvider {
...
class DummyItem : PreviewParameterProvider<EncounterItem> {
  override val values: Sequence<EncounterItem>
    get() = listOf(
      EncounterItem("1", Encounter.EncounterStatus.PLANNED, "Dummy", Date())
    ).asSequence()
}