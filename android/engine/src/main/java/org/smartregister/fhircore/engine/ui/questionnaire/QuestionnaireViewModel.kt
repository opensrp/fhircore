...
constructor(
                ...
                resource.participant =
                    arrayListOf(
                        Encounter.EncounterParticipantComponent()
                            .apply { individual = practitionerRef }
                    )
        }
    }
            ...
            if (questionnaire.isExtractionCandidate()) {
                ...
                bundle.entry.forEach { bun ->
                    ...
                    if (bun.resource.resourceType.isIn(ResourceType.Patient, ResourceType.Group)) {
                        ...
                        if (resourceId == null) questionnaireResponse.subject =
                            bun.resource.asReference()
                    }
                    ...
                    if (questionnaireType.isEditMode()) {
                        if (bun.resource.resourceType.isIn(
    ...
    suspend fun extractCqlOutput(
        ...
        withContext(Dispatchers.Default) {
            val data =
                bundle ?: Bundle().apply { addEntry().apply { resource = questionnaireResponse } }
            questionnaire
                .cqfLibraryIds()
                .map {
                    val patient =
                ...
                .forEach { output ->
                    if (output.isNotEmpty()) extractionProgressMessage.postValue(
                        output.joinToString(
                            "\n"
                        )
                    )
            ...
            ?.let { patientId ->
                loadPatient(patientId)?.apply {
                    if (identifier.isEmpty()) {
                        ...
                        Timber.e(
                            FhirContext.forR4Cached().newJsonParser().encodeResourceToString(this)
                        )
                    }
