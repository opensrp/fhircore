...
class PdfLauncherFragment : DialogFragment() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    val htmlTitle = StringBuilder().append(questionnaireConfig.htmlTitle ?: getString(R.string.default_html_title))
    ...
    lifecycleScope.launch(Dispatchers.IO) {
      ...
      if (patient != null) {
        htmlTitle.append(" - ")
        htmlTitle.append(patient.nameFirstRep.given.joinToString(separator = " "))
        if (patient.nameFirstRep.family != null) {
          htmlTitle.append(" ")
          htmlTitle.append(patient.nameFirstRep.family)
        }
      }

      generatePdf(questionnaireResponse, htmlBinary, htmlTitle.toString())
    }
  }
