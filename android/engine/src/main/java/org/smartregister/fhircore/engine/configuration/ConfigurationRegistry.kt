...
constructor(
    ...
    inline fun <reified T : Configuration> retrieveConfiguration(
        ...
        val configKey =
            if (configType.multiConfig && configId != null) configId else configType.name
            ...
            try {
                ...
                localCompositionResource.run {
                    ...
                    if (iconConfigs.isNotEmpty()) {
                        val ids =
                            iconConfigs.joinToString(DEFAULT_STRING_SEPARATOR) { it.focus.extractId() }
        ...
        if (loadFromAssets) {
            retrieveAssetConfigs(context, appId).forEach { fileName ->
                ...
                if (!fileName.equals(
                    ...
                    val configKey = when {
                       configJson.contains(RESOURCE_TYPE) && (configJson.contains(QUESTIONNAIRE) || configJson.contains(
                           STRUCTUREMAP) || configJson.contains(PLANDEF)) -> configJson.decodeResourceFromString<Resource>().id.extractLogicalIdUuid()
                        else -> fileName
            ...
            composition.retrieveCompositionSections().forEach {
                if (it.hasFocus() && it.focus.hasReferenceElement() && it.focus.hasIdentifier()) {
                    ...
                    val referenceResourceType =
                        it.focus.reference.substringBefore(TYPE_REFERENCE_DELIMITER)
                    ...
                    .forEach { entry: Map.Entry<String, List<Composition.SectionComponent>> ->
                        if (entry.key == ResourceType.List.name) {
                            ...
                            val chunkedResourceIdList =
                                entry.value.chunked(MANIFEST_PROCESSOR_BATCH_SIZE)
    ...
    @VisibleForTesting
    fun isNonProxy(): Boolean = _isNonProxy
            ...
            responseBundle.let {
                bundleEntryComponents.add(
                    Bundle.BundleEntryComponent()
                        .apply { resource = it.entry?.firstOrNull()?.resource },
                )
            }
        }
        return Bundle().apply {
                        ...
                        when (bundleEntryComponent.resource) {
                            is ListResource -> {
                                ...
                                list.entry.forEach { listEntryComponent ->
                                    ...
                                    fhirResourceDataSource.getResource(listResourceUrlPath).entry.forEach { listEntryResourceBundle ->
    ...
    companion object {
        ...
        val QUESTIONNAIRE=  ResourceType.Questionnaire.name
        val STRUCTUREMAP=ResourceType.StructureMap.name
        val PLANDEF=ResourceType.PlanDefinition.name
