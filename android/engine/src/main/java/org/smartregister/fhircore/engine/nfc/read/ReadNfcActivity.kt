...
// This activity inherit NFC Activity, that is a non-UI Activity and contains all the NFC
// implementation
class ReadNfcActivity : NFCActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    // Listening to the event Flow that will be consumed only once, in order to perform some change
    // in the UI
    // Here we will show some SUCCESS or ERROR Dialog depending on the result
    lifecycleScope.launchWhenStarted {
      DESFireServiceAccess.eventFlow.collect { event ->
        if (event == CardReaderEvent.READ_SUCCESS) {
          // Dialog OK
          dialog =
            FamocoDialog(this@ReadNfcActivity)
              .setDialogType(DialogType.SUCCESS)
              .setTitle(getString(R.string.success))
              .cancelable(true)
              .setContent(getString(R.string.read_success_content))
        ...
        if (event.name.contains("ERROR")) {
          ...
          dialog =
            FamocoDialog(this@ReadNfcActivity)
              ...
              .cancelable(true)
              .setContent(
                String.format(getString(R.string.read_error_content_format), event.name, event.code)
              )
              .showPositiveButton(false)
              .setOnNegativeButtonClicked(getString(R.string.close)) {
                dialog.dismiss()
  ...
  /**
   * You need also to pass to this activity the proto file that will be used in order to perform NFC
   * actions Here the value is already generated in the MainActivity, so we are getting it from the
   * ViewModel that obtain it from the DataProvider object
   *
   * @return the proto file that will be used
   */
  override fun getProtoFile(): ByteArray {
    return readViewModel.protoFile.toByteArray()
  }

  /**
   * Here the value is already generated in the MainActivity, so we are getting it from the
   * WriteViewModel that obtain it from the DataProvider object
   *
   * @return The file Number on the card to interact with
   */
  override fun getFileNumber(): String {
    return readViewModel.fileNumber
  }

  /**
   * For the NfcMode.WRITE, you will need to pass the data to write in the card Here the value is
   * already generated in the MainActivity, so we are getting it from the WriteViewModel that obtain
   * it from the DataProvider object
   *
   * @return the data that will be written in the card (if needed)
   */
  override fun getJsonDataToWrite(): String? {
    return readViewModel.jsonDataToWrite
  }
}
