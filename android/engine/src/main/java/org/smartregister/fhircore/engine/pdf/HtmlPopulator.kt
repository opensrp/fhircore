 ...
 *
 * @param questionnaireResponse The [QuestionnaireResponse] object containing the data to be
 *   populated in the HTML.
 */
class HtmlPopulator(
  ...
  fun populateHtml(html: String): String {
    return html.processIsNotEmpty().populateAnswerAsList().populateAnswer().populateSubmittedDate()
  }
  ...
  private fun String.processIsNotEmpty(): String {
    ...
    while (html.contains("@is-not-empty('")) {
      ...
      val htmlWithoutTag =
        html.substringAfter("@is-not-empty('$linkId')").substringBefore("@is-not-empty")
      html =
        if (questionnaireResponseItemMap.getValue(linkId).isNotEmpty()) {
  ...
  private fun String.populateAnswerAsList(): String {
    ...
    while (html.contains("@answer-as-list('")) {
      ...
      val answerAsList =
        questionnaireResponseItemMap.getValue(linkId).joinToString(separator = "") { answer ->
  ...
  private fun String.populateAnswer(): String {
    ...
    while (html.contains("@answer('")) {
      ...
      val answer =
        questionnaireResponseItemMap.getValue(linkId).joinToString { answer ->
