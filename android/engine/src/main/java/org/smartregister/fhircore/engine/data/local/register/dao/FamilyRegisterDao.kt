...
constructor(
  ...
  override suspend fun loadProfileData(appFeatureName: String?, resourceId: String): ProfileData {
    ...
    return ProfileData.FamilyProfileData(
      ...
      members =
        family.member?.map { member ->
          fhirEngine.load(Patient::class.java, member.entity.extractId()).let { patient ->
            loadFamilyMemberProfileData(patient.logicalId)
          }
        }
          ?: listOf(),
      services =
      ...
      tasks =
        fhirEngine.search<Task> {
          filterByResourceTypeId(Task.SUBJECT, ResourceType.Group, family.logicalId)
        }
    )
  }
  ...
  private suspend fun loadFamilyMemberRegisterData(memberId: String) =
    fhirEngine.load(Patient::class.java, memberId).let { patient ->
  ...
  private suspend fun loadMemberTask(patientId: String) =
    fhirEngine.search<Task> {
      filterByResourceTypeId(Task.SUBJECT, ResourceType.Patient, patientId)
    }

  private suspend fun loadMemberFlags(patientId: String) =
    fhirEngine.search<Flag> {
      filterByResourceTypeId(Flag.SUBJECT, ResourceType.Patient, patientId)
    }
