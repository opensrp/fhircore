...
class CompositionSyncWorkerTest : RobolectricTest() {
  @Test
  fun `should create sync worker with expected properties`() {
    ...
    every { sharedPreferencesHelper.read(SharedPreferenceKey.APP_ID.name, "") } returns "appId"
    coEvery { sharedPreferencesHelper.read(SharedPreferenceKey.APP_ID.name, "") } returns "appId"

    val testSyncWorker =
      CompositionSyncWorker(mockk(), workerParams, fhirEngine, timeContext, mockk(), syncParamSource)

    //testSyncWorker.getDownloadWorkManager()
    //verify { testSyncWorker.getDownloadWorkManager() }
