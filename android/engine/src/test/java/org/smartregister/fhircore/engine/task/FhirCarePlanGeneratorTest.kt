...
class FhirCarePlanGeneratorTest : RobolectricTest() {
    ...
    @Inject
    lateinit var fhirPathEngine: FHIRPathEngine
    ...
    fun testGenerateCarePlanForPatient() = runTest {
        val planDefinition = "plans/child-routine-visit/plandefinition.json".readFile().decodeResourceFromString<PlanDefinition>()
        ...
        val structureMap = structureMapUtilities.parse(structureMapScript, "ChildRoutineCarePlan").also {
        ...
        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = patient }))!!.also { println(it.encodeResourceToString()) }.also { carePlan ->
            ...
            Assert.assertEquals("This defines the schedule of care for patients under 5 years old", carePlan.description)
    ...
    fun testGenerateCarePlanForDiabetesClient() = runTest {
        ...
        val questionnaireResponse = "plans/diabetes_compass/patient_screening_questionnaire_low_risk_response.json".readFile().decodeResourceFromString<QuestionnaireResponse>()

        val patient = "plans/child-routine-visit/sample/patient.json".readFile().decodeResourceFromString<Patient>()

        val structureMapScript = "plans/diabetes_compass/three-year-routine-screening-follow-up.map".readFile()
        ...
        val booleanSlot = slot<Boolean>()
        coEvery { defaultRepository.create(capture(booleanSlot), capture(resourcesSlot)) } returns emptyList()
        coEvery { fhirEngine.get<StructureMap>("dc-3-year-routine-screening-sm") } returns structureMap
        coEvery { fhirEngine.search<CarePlan>(Search(ResourceType.CarePlan)) } returns listOf()
    ...
    fun testGenerateCarePlanForGroup() = runTest {
        val planDefinition = "plans/household-routine-visit/plandefinition.json".readFile().decodeResourceFromString<PlanDefinition>()

        val group = "plans/household-routine-visit/sample/group.json".readFile().decodeResourceFromString<Group>()

        val structureMapScript = "plans/household-routine-visit/structure-map.txt".readFile()
        val structureMap = structureMapUtilities.parse(structureMapScript, "HHRoutineCarePlan").also {
            // TODO: IMP - The parser does not recognize the time unit i.e. months and prints as ''
            //  so use only months and that would have the unit replaced with 'months'
            println(it.encodeResourceToString().replace("''", "'month'"))
        }
        ...
        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, group, Bundle().addEntry(Bundle.BundleEntryComponent().apply {
            resource = Encounter().apply { status = Encounter.EncounterStatus.FINISHED }
        }))!!.also { println(it.encodeResourceToString()) }.also { carePlan ->
            Assert.assertNotNull(UUID.fromString(carePlan.id))
            Assert.assertEquals(CarePlan.CarePlanStatus.ACTIVE, carePlan.status)
            Assert.assertEquals(CarePlan.CarePlanIntent.PLAN, carePlan.intent)
    ...
    fun testGenerateCarePlanForHouseHold() = runTest {
        val planDefinition = "plans/household-wash-check-routine-visit/plandefinition.json".readFile().decodeResourceFromString<PlanDefinition>()

        val group = "plans/household-wash-check-routine-visit/sample/group.json".readFile().decodeResourceFromString<Group>()
        ...
        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, group, Bundle().addEntry(Bundle.BundleEntryComponent().apply {
            ...
            Assert.assertEquals("This defines the schedule of service for WASH Check on households", carePlan.description)
            Assert.assertEquals(group.logicalId, carePlan.subject.extractId())
            Assert.assertEquals(DateTimeType.now().value.makeItReadable(), carePlan.created.makeItReadable())
            Assert.assertNotNull(carePlan.period.start)
            Assert.assertTrue(carePlan.activityFirstRep.outcomeReference.isNotEmpty())

            resourcesSlot.filter { res -> res.resourceType == ResourceType.Task }.map { it as Task }.also { list -> Assert.assertTrue(list.isNotEmpty() && list.size > 59 && list.size < 62) }.all { task ->
                task.status == TaskStatus.REQUESTED && LocalDate.parse(task.executionPeriod.end.asYyyyMmDd()).let { localDate ->
                    localDate.dayOfMonth == localDate.lengthOfMonth()
                }
            }
    ...
    fun testGenerateCarePlanForSickChildOver2m() = runTest {
        val planDefinitionResources = loadPlanDefinitionResources("sick-child-visit", listOf("register-over2m"))
        ...
        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponses.first() }))!!.also { println(it.encodeResourceToString()) }.also { carePlan ->
            assertCarePlan(carePlan, planDefinition, patient, Date(), Date().plusDays(7), 3)

            resourcesSlot.filter { res -> res.resourceType == ResourceType.Task }.map { it as Task }.also { assertEquals(4, it.size) } // 4 tasks generated, 3 followup 1 referral
                    .also {
                        ...
                        assertTrue(this.all { task ->
                            task.executionPeriod.end.asYyyyMmDd() == Date().plusDays(7).asYyyyMmDd()
                        })
                        assertTrue(this.all { it.basedOn.first().reference == carePlan.asReference().reference })
                        assertTrue(this.elementAt(0).executionPeriod.start.asYyyyMmDd() == Date().plusDays(1).asYyyyMmDd())
                        assertTrue(this.elementAt(1).executionPeriod.start.asYyyyMmDd() == Date().plusDays(2).asYyyyMmDd())
                        assertTrue(this.elementAt(2).executionPeriod.start.asYyyyMmDd() == Date().plusDays(3).asYyyyMmDd())
                    }
        }
    }
    ...
    fun testGenerateCarePlanForSickChildUnder2m() = runTest {
        val planDefinitionResources = loadPlanDefinitionResources("sick-child-visit", listOf("register-under2m"))
        val planDefinition = planDefinitionResources.planDefinition
        val patient = planDefinitionResources.patient.apply { this.birthDate = Date().plusMonths(-1) }
        val questionnaireResponses = planDefinitionResources.questionnaireResponses
        val resourcesSlot = planDefinitionResources.resourcesSlot

        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponses.first() })).also { carePlan ->
            Assert.assertNull(carePlan)
    ...
    fun testCompleteCarePlanForSickChildFollowup() = runTest {
        val plandefinition = "plans/sick-child-visit/plandefinition.json".readFile().decodeResourceFromString<PlanDefinition>()

        val patient = "plans/sick-child-visit/sample/patient.json".readFile().decodeResourceFromString<Patient>()
        val questionnaireResponse = "plans/sick-child-visit/sample/questionnaire-response-followup.json".readFile().decodeResourceFromString<QuestionnaireResponse>()

        val structureMapReferral = structureMapUtilities.parse("plans/structure-map-referral.txt".readFile(), "ReferralTask").also { println(it.encodeResourceToString()) }

        val createdTasksSlot = mutableListOf<Resource>()
        val updatedTasksSlot = mutableListOf<Resource>()
        val booleanSlot = slot<Boolean>()
        coEvery { defaultRepository.create(capture(booleanSlot), capture(createdTasksSlot)) } returns emptyList()
        coEvery { defaultRepository.addOrUpdate(any(), capture(updatedTasksSlot)) } just Runs
        coEvery { fhirEngine.update(any()) } just runs
        coEvery { fhirEngine.get<StructureMap>("528a8603-2e43-4a2e-a33d-1ec2563ffd3e") } returns structureMapReferral

        coEvery { fhirEngine.search<CarePlan>(Search(ResourceType.CarePlan)) } returns listOf(CarePlan().apply {
        ...
        coEvery { fhirEngine.get<Task>(any()) } returns Task().apply {
            id = "1111"
            status = TaskStatus.READY
        }

        fhirCarePlanGenerator.generateOrUpdateCarePlan(plandefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponse }))!!.also { carePlan: CarePlan ->
            ...
            createdTasksSlot.also { list -> Assert.assertTrue(list.size == 2) }.filter { resource -> resource.resourceType == ResourceType.Task }.map { resource -> resource as Task }.also { tasks ->
                tasks.first().let { task ->
                    Assert.assertTrue(task.status == TaskStatus.READY)
                    Assert.assertTrue(task.basedOn.first().reference == plandefinition.asReference().reference)
    ...
    fun testUpdateCarePlanForSickChildReferral() = runTest {
        ...
        val patient = "plans/sick-child-visit/sample/patient.json".readFile().decodeResourceFromString<Patient>().apply { this.birthDate = Date().plusMonths(-1).plusDays(-15) }

        val questionnaireResponse = "plans/sick-child-visit/sample/questionnaire-response-register-under2m.json".readFile().decodeResourceFromString<QuestionnaireResponse>()

        val structureMapScript = "plans/structure-map-referral.txt".readFile()
        val structureMap = structureMapUtilities.parse(structureMapScript, "ReferralTask").also {
        ...
        val booleanSlot = slot<Boolean>()
        coEvery { defaultRepository.create(capture(booleanSlot), capture(resourcesSlot)) } returns emptyList()
        coEvery { fhirEngine.get<StructureMap>("528a8603-2e43-4a2e-a33d-1ec2563ffd3e") } returns structureMap

        coEvery { fhirEngine.search<CarePlan>(Search(ResourceType.CarePlan)) } returns listOf()

        fhirCarePlanGenerator.generateOrUpdateCarePlan(plandefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponse })).also { _ ->
            resourcesSlot.forEach { println(it.encodeResourceToString()) }

            resourcesSlot.filter { it.resourceType == ResourceType.Task }.map { it as Task }.also { list -> Assert.assertTrue(list.size == 1) }.also {
                it.first().let {
                    Assert.assertTrue(it.status == TaskStatus.READY)
                    Assert.assertTrue(it.basedOn.first().reference == plandefinition.asReference().reference)
                    Assert.assertTrue(it.`for`.reference == patient.asReference().reference)
    ...
    @Test
    fun `generateOrUpdateCarePlan should generate full careplan for 8 visits when lmp is today`() = runTest {
        ...
        questionnaireResponses.first().find("245679f2-6172-456e-8ff3-425f5cea3243")!!.answer.first().value = DateType(lmp)

        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponses.first() }))!!.also { println(it.encodeResourceToString()) }.also {
            val carePlan = it
            assertCarePlan(carePlan, planDefinition, patient, lmp, lmp.plusMonths(9), 8) // 8 visits for each month of ANC

            resourcesSlot.forEach { println(it.encodeResourceToString()) }

            assertTrue(resourcesSlot.first() is CarePlan)

            resourcesSlot.filter { res -> res.resourceType == ResourceType.Task }.map { it as Task }.also { assertEquals(9, it.size) } // 8 for visit, 1 for referral
                    .also {
                        assertTrue(it.all { it.status == TaskStatus.READY })
                        assertTrue(it.all { it.`for`.reference == patient.asReference().reference })
                    }
                    ...
                    .take(8).run {
                        ...
                        this.forEach { task ->
                            fhirPathEngine.evaluate(pregnancyStart, "\$this + 1 'month'").firstOrNull()?.dateTimeValue()?.let { result -> pregnancyStart = result }
                            assertEquals(pregnancyStart.valueToString(), DateTimeType(task.executionPeriod.start).valueToString())
                        }
                    }
        }
    }
    ...
    @Test
    fun `generateOrUpdateCarePlan should generate careplan for 5 visits when lmp has passed 3 months`() = runTest {
        val planDefinitionResources = loadPlanDefinitionResources("anc-visit", listOf("register"))
        val planDefinition = planDefinitionResources.planDefinition
        val patient = planDefinitionResources.patient
        val questionnaireResponses = planDefinitionResources.questionnaireResponses
        ...
        questionnaireResponses.first().find("245679f2-6172-456e-8ff3-425f5cea3243")!!.answer.first().value = lmp

        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponses.first() }))!!.also { println(it.encodeResourceToString()) }.also { resourcesSlot.forEach { println(it.encodeResourceToString()) } }.also { carePlan ->
            assertCarePlan(carePlan, planDefinition, patient, lmp.value, lmp.value.plusMonths(9), 5) // 5 visits for each month of ANC

            resourcesSlot.filter { res -> res.resourceType == ResourceType.Task }.map { it as Task }.also { assertEquals(6, it.size) } // 5 for visit, 1 for referral
                    .also {
                        assertTrue(it.all { it.status == TaskStatus.READY })
                        assertTrue(it.all { it.`for`.reference == patient.asReference().reference })
                    }
                    ...
                    .take(5).run {
                        ...
                        this.forEachIndexed { index, task ->
                            assertEquals(ancStart.plusMonths(index + 1).asYyyyMmDd(), task.executionPeriod.start.asYyyyMmDd())
                        }
                    }
        }
    }
    ...
    @Test
    fun `generateOrUpdateCarePlan should generate careplan for next visit when ondemand task is required`() = runTest {
        val planDefinitionResources = loadPlanDefinitionResources("anc-visit-ondemand", listOf("register"))
        val planDefinition = planDefinitionResources.planDefinition
        val patient = planDefinitionResources.patient
        val questionnaireResponses = planDefinitionResources.questionnaireResponses
        val resourcesSlot = planDefinitionResources.resourcesSlot
        ...
        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponses.first() }))!!.also { println(it.encodeResourceToString()) }.also { carePlan ->
            assertCarePlan(carePlan, planDefinition, patient, lmp, lmp.plusMonths(9), 1) // 1 visits for next month of ANC

            resourcesSlot.forEach { println(it.encodeResourceToString()) }
            assertEquals(1, carePlan.activityFirstRep.outcomeReference.size)

            resourcesSlot.filter { res -> res.resourceType == ResourceType.Task }.map { it as Task }.also { assertEquals(2, it.size) } // 1 for visit, 1 for referral
                    .also { tasks ->
                    ...
                    .take(1).run {
                        ...
                        this.forEachIndexed { _, task ->
                            assertEquals(Date().plusMonths(1).asYyyyMmDd(), task.executionPeriod.start.asYyyyMmDd())
                        }
                    }
        }
    }
    ...
    fun `Generate CarePlan should generate child immunization schedule`() = runTest {
        val planDefinitionResources = loadPlanDefinitionResources("child-immunization-schedule", listOf("register-temp"))
        val planDefinition = planDefinitionResources.planDefinition
        val patient = planDefinitionResources.patient
        val questionnaireResponses = planDefinitionResources.questionnaireResponses
        val resourcesSlot = planDefinitionResources.resourcesSlot

        fhirCarePlanGenerator.generateOrUpdateCarePlan(planDefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = patient }).addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponses.first() }))!!.also { println(it.encodeResourceToString()) }.also { carePlan ->
            assertCarePlan(carePlan, planDefinition, patient, patient.birthDate, patient.birthDate.plusDays(4017), 20)

            resourcesSlot.filter { res -> res.resourceType == ResourceType.Task }.map {
                println(it.encodeResourceToString())
                it as Task
            }.also { tasks ->
                ...
                assertTrue(tasks.all {
                    it.code.codingFirstRep.display == "Administration of vaccine to produce active immunity (procedure)" && it.code.codingFirstRep.code == "33879002"
                })
                assertTrue(tasks.all { it.description.contains(it.reasonCode.text, true) })
                assertTrue(tasks.all { it.`for`.reference == questionnaireResponses.first().subject.reference })
                assertTrue(tasks.all { it.basedOnFirstRep.reference == carePlan.asReference().reference })
            }.also { tasks ->
                val vaccines = mutableMapOf<String, Date>(
    ...
    @Test
    @ExperimentalCoroutinesApi
    fun `test generateOrUpdateCarePlan returns success even when evaluatedValue is null`() = runBlocking {
        val planDefinitionResources = loadPlanDefinitionResources("child-immunization-schedule", listOf("register-temp"))
        val questionnaireResponses = planDefinitionResources.questionnaireResponses
        val planDefinition = planDefinitionResources.planDefinition
        val patient = planDefinitionResources.patient
        val data = Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = patient }).addEntry(Bundle.BundleEntryComponent().apply { resource = questionnaireResponses.first() })
    ...
    @Test
    @ExperimentalCoroutinesApi
    fun `Generate CarePlan should generate child immunization schedule with correct groupIdentifier value`() = runTest {
        val planDefinitionResources = loadPlanDefinitionResources("child-immunization-schedule", listOf("register-temp"))
        val planDefinition = planDefinitionResources.planDefinition
        val patient = planDefinitionResources.patient
        val questionnaireResponses = planDefinitionResources.questionnaireResponses
    ...
    @Test
    @ExperimentalCoroutinesApi
    fun `Generate CarePlan should generate child immunization schedule with pre-req def and expiry timing`() = runTest {
        val planDefinitionResources = loadPlanDefinitionResources("child-immunization-schedule", listOf("register-temp"))
        val planDefinition = planDefinitionResources.planDefinition
        val patient = planDefinitionResources.patient
        val questionnaireResponses = planDefinitionResources.questionnaireResponses
    ...
    fun `Generate CarePlan should generate disease followup schedule`() = runTest {
        val plandefinition = "plans/disease-followup/plan-definition.json".readFile().decodeResourceFromString<PlanDefinition>()

        val patient = "plans/disease-followup/patient.json".readFile().decodeResourceFromString<Patient>()
        val diseaseFollowUpQuestionnaireResponseString = "plans/disease-followup/questionnaire-response.json".readFile().decodeResourceFromString<QuestionnaireResponse>()

        val structureMapScript = "plans/disease-followup/structure-map.txt".readFile()
        val structureMap = structureMapUtilities.parse(structureMapScript, "eCBIS Child Immunization").also {
            println(it.encodeResourceToString())
        }
        ...
        val booleanSlot = slot<Boolean>()
        coEvery { defaultRepository.create(capture(booleanSlot), capture(resourcesSlot)) } returns emptyList()
        coEvery { fhirEngine.get<StructureMap>("63752b18-9f0e-48a7-9a21-d3714be6309a") } returns structureMap
        coEvery { fhirEngine.search<CarePlan>(Search(ResourceType.CarePlan)) } returns listOf()
        fhirCarePlanGenerator.generateOrUpdateCarePlan(plandefinition, patient, Bundle().addEntry(Bundle.BundleEntryComponent().apply { resource = patient }).addEntry(Bundle.BundleEntryComponent().apply {
            ...
            Assert.assertEquals("This is a follow up for patient's marked with the following diseases HIV, TB, Mental Health & CM-NTD", carePlan.description)
            Assert.assertEquals(patient.logicalId, carePlan.subject.extractId())
            Assert.assertEquals(DateTimeType.now().value.makeItReadable(), carePlan.created.makeItReadable())
            Assert.assertEquals(patient.generalPractitionerFirstRep.extractId(), carePlan.author.extractId())
            Assert.assertTrue(carePlan.activityFirstRep.outcomeReference.isNotEmpty())
            coEvery { defaultRepository.create(capture(booleanSlot), capture(resourcesSlot)) }
            resourcesSlot.filter { res -> res.resourceType == ResourceType.Task }.map { it as Task }.also { list -> Assert.assertTrue(list.isNotEmpty()) }.also { println(it.last().encodeResourceToString()) }.all { task ->
                task.status == TaskStatus.INPROGRESS && LocalDate.parse(task.executionPeriod.end.asYyyyMmDd()).let { localDate ->
                    localDate.dayOfMonth == localDate.lengthOfMonth()
                }
            }
        }
    ...
    fun testConditionallyUpdateCarePlanStatusTerminatesWhenNoCarePlanIsFound() {
        ...
        val questionnaireConfig: QuestionnaireConfig = QuestionnaireConfig(id = "id-1", planDefinitions = planDefinitions, carePlanConfigs = listOf(carePlanConfig))
        ...
        coEvery {
            fhirEngine.search<CarePlan> {
                filter(CarePlan.INSTANTIATES_CANONICAL, { value = "${PlanDefinition().fhirType()}/plandef-1" })
                filter(CarePlan.SUBJECT, { value = patient.referenceValue() })
            }
        } returns listOf()

        runBlocking {
            fhirCarePlanGenerator.conditionallyUpdateCarePlanStatus(questionnaireConfig = questionnaireConfig, subject = patient, bundle = bundle)
        }
    ...
    fun testConditionallyUpdateCarePlanStatusRevokesCarePlanWhenCarePlanStatusIsActive() {
        val planDefinitions = listOf("plandef-1")
        val carePlanConfig = CarePlanConfig(fhirPathExpression = "Patient.active and %resource.entry.where(resource is QuestionnaireResponse).resource.where(questionnaire = 'Questionnaire/450cb100-0c5b-47c6-9f33-2830a79be726').exists()")
        val questionnaireConfig: QuestionnaireConfig = QuestionnaireConfig(id = "id-1", planDefinitions = planDefinitions, carePlanConfigs = listOf(carePlanConfig))
        val patient = Patient().apply {
            id = "patient-1"
            active = true
        ...
        val bundle = Bundle().apply {
            ...
            entry = listOf(Bundle.BundleEntryComponent().apply { resource = patient }, Bundle.BundleEntryComponent().apply { resource = questionnaireResponse })
        }
        coEvery {
            fhirEngine.search<CarePlan> {
                filter(CarePlan.INSTANTIATES_CANONICAL, { value = "${PlanDefinition().fhirType()}/plandef-1" })
    ...
    fun testConditionallyUpdateCarePlanStatusCancelsTasks() {
        ...
        val carePlan = CarePlan().apply {
            id = "careplan-1"
            status = CarePlan.CarePlanStatus.ACTIVE
            activity = listOf(CarePlanActivityComponent().apply {
    ...
    data class PlanDefinitionResources(val planDefinition: PlanDefinition, val patient: Patient, val questionnaireResponses: List<QuestionnaireResponse>, val structureMap: StructureMap, val structureMapReferral: StructureMap, val resourcesSlot: MutableList<Resource>)

    fun loadPlanDefinitionResources(planName: String, questionnaireResponseTags: List<String> = emptyList()): PlanDefinitionResources {
        val planDefinition = "plans/$planName/plandefinition.json".readFile().decodeResourceFromString<PlanDefinition>()

        val patient = "plans/$planName/sample/patient.json".readFile().decodeResourceFromString<Patient>()

        val questionnaireResponses = questionnaireResponseTags.map {
            "plans/$planName/sample/questionnaire-response-$it.json".readFile().decodeResourceFromString<QuestionnaireResponse>()
        }

        val structureMapRegister = structureMapUtilities.parse("plans/$planName/structure-map-register.txt".readFile(), "${planName.uppercase().replace("-", "").replace(" ", "")}CarePlan").also { println(it.encodeResourceToString()) }
        ...
        coEvery { defaultRepository.create(capture(booleanSlot), capture(resourcesSlot)) } returns emptyList()
        coEvery { fhirEngine.search<CarePlan>(Search(ResourceType.CarePlan)) } returns listOf()
        coEvery { fhirEngine.get<StructureMap>(structureMapRegister.logicalId) } returns structureMapRegister
        coEvery { fhirEngine.get<StructureMap>("528a8603-2e43-4a2e-a33d-1ec2563ffd3e") } returns structureMapReferral

        return PlanDefinitionResources(planDefinition, patient, questionnaireResponses, structureMapRegister, structureMapReferral, resourcesSlot)
    }

    fun assertCarePlan(carePlan: CarePlan, planDefinition: PlanDefinition, patient: Patient, referenceDate: Date, endDate: Date, visitTasks: Int) {
    ...
    fun `test updateDependentTaskDueDate - dependent task without output`() {
        coEvery { fhirEngine.get(ResourceType.Task, "650203d2-f327-4eb4-a9fd-741e0ce29c3f") } returns groupTask.apply {
            status = TaskStatus.READY
            partOf = listOf(Reference("Task/650203d2-f327-4eb4-a9fd-741e0ce29c3f"))
        }
        dependentTask.apply {
            output = emptyList()
            executionPeriod = Period().apply {
                start = Date()
                end = Date()
            }
        }
    ...
    fun `test updateDependentTaskDueDate with dependent task , with output but no execution period start date`() {
        coEvery { fhirEngine.get(ResourceType.Task, "650203d2-f327-4eb4-a9fd-741e0ce29c3f") } returns groupTask.apply { status = TaskStatus.INPROGRESS }
    ...
    fun `test updateDependentTaskDueDate with dependent task with output, execution period start date, and encounter part of reference that is null`() {
        ...
        coEvery { fhirEngine.get(ResourceType.Task, "650203d2-f327-4eb4-a9fd-741e0ce29c3f") } returns dependentTask.apply {
            status = TaskStatus.INPROGRESS
            output = listOf(Task.TaskOutputComponent(CodeableConcept(), StringType("{\n" + "          \"reference\": \"Encounter/14e2ae52-32fc-4507-8736-1177cdaafe90\"\n" + "        }")))
        }
        coEvery {
            defaultRepository.loadResource(Reference(Json.decodeFromString<JsonObject>(dependentTask.output.first().value.toString())[REFERENCE]?.jsonPrimitive?.content))
        } returns encounter.apply { partOf = null }
        coEvery { defaultRepository.loadResource(Reference(ArgumentMatchers.anyString())) } returns Immunization()
        val updatedTask = runBlocking { groupTask.updateDependentTaskDueDate(defaultRepository) }
        assertEquals(groupTask, updatedTask)
    }
    ...
    fun `test updateDependentTaskDueDate with dependent task with output, execution period start date, and encounter part of reference that is not an Immunization`() {
        ...
        coEvery { defaultRepository.loadResource(Reference(encounter.partOf.reference)) } returns immunizationResource
    ...
    fun `updateDependentTaskDueDate sets executionPeriod start correctly`() {
        ...
        assertEquals(Date.from(Instant.parse("2021-11-07T00:00:00Z")), dependentTask.executionPeriod.start)
