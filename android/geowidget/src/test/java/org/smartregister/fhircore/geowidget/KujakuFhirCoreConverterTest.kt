...
class KujakuFhirCoreConverterTest {
  ...
  fun testCheckConversionGuideShouldInitializePrivateConversionGuide() {
    ...
    Assert.assertNotNull(kujakuConverter.checkConversionGuide(ApplicationProvider.getApplicationContext()))
  }
  ...
  fun testFetchConversionGuide() {
    ...
    val conversionGuide = ReflectionHelpers.getField<HashMap<String, LinkedList<Pair<String, String>>>>(kujakuConverter, "conversionGuide")
  ...
  fun testGenerateFeatureCollectionFromLocationWithBoundaryGeojsonExt() {
    ...
    val location = FhirContext.forR4().newJsonParser().parseResource(Location::class.java, locationJson)
    ...
    resourceGroups.add(ArrayList<Resource>().apply {
      add(Group().apply {
        name = "John Doe Fam"
        id = "90230823"
      })
      add(Location().apply {
        ...
        extension = listOf(Extension(KujakuFhirCoreConverter.BOUNDARY_GEOJSON_EXT_URL).apply {
          setValue(Attachment().apply {
            contentType = "application/geo+json"
            data = Base64.encodeBase64("""{"type":"Feature","properties":{},"geometry":{"type":"Point","coordinates":[12.83203125,28.304380682962783]}}""".encodeToByteArray())
          })
        })
      })
    })
    ...
    val featureCollection = kujakuConverter.generateFeatureCollection(ApplicationProvider.getApplicationContext(), resourceGroups)

    Assert.assertEquals("John Doe Fam", featureCollection.features()!![0].getStringProperty("name"))
    Assert.assertEquals("90230823", featureCollection.features()!![0].getStringProperty("family-id"))
    Assert.assertEquals(28.304380682962783, (featureCollection.features()!![0].geometry() as Point).latitude(), 0.0)
    Assert.assertEquals(12.83203125, (featureCollection.features()!![0].geometry() as Point).longitude(), 0.0)

    Assert.assertEquals("new family", featureCollection.features()!![1].getStringProperty("name"))
    Assert.assertEquals("Group/1122f50c-5499-4eaa-bd53-a5364371a2ba/_history/5", featureCollection.features()!![1].getStringProperty("family-id"))
    Assert.assertEquals(-1.301070677485388, (featureCollection.features()!![1].geometry() as Point).latitude(), 0.0)
    Assert.assertEquals(36.80826008319855, (featureCollection.features()!![1].geometry() as Point).longitude(), 0.0)
  }
  ...
  fun testFeatureGeneration() {
    ...
    val conversionInterface = KujakuFhirCoreConverter()
