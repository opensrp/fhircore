...
class AncListViewModelTest : RobolectricTest() {

  private lateinit var viewModel: AncListViewModel
  ...
  fun setUp() {
    ...
    viewModel = spyk(AncListViewModel(ApplicationProvider.getApplicationContext(), fhirEngine))
  }
  ...
  fun testSearchResultsPatientsFilter() {
    ...
    Assert.assertEquals("Pregnant", filters[0].value)
  ...
  private fun makePatient(): Patient {
    return Patient().apply { name.add(HumanName().apply { this.family = "abc" }) }
  }
}
